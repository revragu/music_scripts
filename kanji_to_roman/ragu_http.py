...
def httpReq(address_root,req,*args):
    backoff_time=5
    address_sub=''.join(args)
    for i in range(1,4):
        try:
            resp=requests.get(address_root + address_sub, req,timeout=30)
            if resp.status_code == 200:
                return(json.loads(resp.content),resp.status_code)
            return(False,resp.status_code)
            
        except requests.exceptions.ReadTimeout or requests.exceptions.Timeout or requests.exceptions.ConnectionError:
            backoff_time=backoff_time * i
            time.sleep(backoff_time)
       
        except Exception as e:
            raise Exception(f"Unhandled exception of type {str(type(e))} in httpReq: {str(e)}")
